...
typedef struct __mavlink_vfr_hud_t {
 ...
 uint32_t engine_speed; /*< [c/m] Engine speed (0 to 10000).*/
...
#define MAVLINK_MSG_ID_VFR_HUD_MIN_LEN 24
#define MAVLINK_MSG_ID_74_LEN 24
#define MAVLINK_MSG_ID_74_MIN_LEN 24

#define MAVLINK_MSG_ID_VFR_HUD_CRC 122
#define MAVLINK_MSG_ID_74_CRC 122
...
#define MAVLINK_MESSAGE_INFO_VFR_HUD { \
    ...
    {  { "airspeed", NULL, MAVLINK_TYPE_FLOAT, 0, 0, offsetof(mavlink_vfr_hud_t, airspeed) }, \
         ...
         { "heading", NULL, MAVLINK_TYPE_INT16_T, 0, 20, offsetof(mavlink_vfr_hud_t, heading) }, \
         { "throttle", NULL, MAVLINK_TYPE_UINT16_T, 0, 22, offsetof(mavlink_vfr_hud_t, throttle) }, \
         { "engine_speed", NULL, MAVLINK_TYPE_UINT32_T, 0, 8, offsetof(mavlink_vfr_hud_t, engine_speed) }, \
 ...
 * @param engine_speed [c/m] Engine speed (0 to 10000).
...
static inline uint16_t mavlink_msg_vfr_hud_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               float airspeed, float groundspeed, int16_t heading, uint16_t throttle, uint32_t engine_speed, float alt, float climb)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_VFR_HUD_LEN];
    _mav_put_float(buf, 0, airspeed);
    _mav_put_float(buf, 4, groundspeed);
    _mav_put_uint32_t(buf, 8, engine_speed);
    _mav_put_float(buf, 12, alt);
    _mav_put_float(buf, 16, climb);
    _mav_put_int16_t(buf, 20, heading);
    _mav_put_uint16_t(buf, 22, throttle);
...
{
    return mavlink_msg_vfr_hud_pack(system_id, component_id, msg, vfr_hud->airspeed, vfr_hud->groundspeed, vfr_hud->heading, vfr_hud->throttle, vfr_hud->engine_speed, vfr_hud->alt, vfr_hud->climb);
}
...
{
    return mavlink_msg_vfr_hud_pack_chan(system_id, component_id, chan, msg, vfr_hud->airspeed, vfr_hud->groundspeed, vfr_hud->heading, vfr_hud->throttle, vfr_hud->engine_speed, vfr_hud->alt, vfr_hud->climb);
}
...
static inline void mavlink_msg_vfr_hud_send(mavlink_channel_t chan, float airspeed, float groundspeed, int16_t heading, uint16_t throttle, uint32_t engine_speed, float alt, float climb)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_VFR_HUD_LEN];
    _mav_put_float(buf, 0, airspeed);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_vfr_hud_send(chan, vfr_hud->airspeed, vfr_hud->groundspeed, vfr_hud->heading, vfr_hud->throttle, vfr_hud->engine_speed, vfr_hud->alt, vfr_hud->climb);
#else
 ...
 */
static inline void mavlink_msg_vfr_hud_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  float airspeed, float groundspeed, int16_t heading, uint16_t throttle, uint32_t engine_speed, float alt, float climb)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field engine_speed from vfr_hud message
 *
 * @return [c/m] Engine speed (0 to 10000).
 */
static inline uint32_t mavlink_msg_vfr_hud_get_engine_speed(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_vfr_hud_decode(const mavlink_message_t* msg, mavlink_vfr_hud_t* vfr_hud)
{
    ...
    vfr_hud->engine_speed = mavlink_msg_vfr_hud_get_engine_speed(msg);
